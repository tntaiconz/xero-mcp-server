...
class CustomConnectionsXeroClient extends XeroClient {
  ...
  constructor(config: { clientId: string; clientSecret: string; grantType: string }) {
    super(config);
  ...
  protected buildHeaders(bearerToken: string, contentType = 'application/json'): { [key: string]: string } {
    return {
