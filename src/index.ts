...
import { listXeroContacts, listXeroInvoices, createXeroInvoice } from './xero/tools.js';
      ...
      content: [
        ...
        ...result.contacts?.map(contact => ({
          ...
          text: [
            ...
            contact.bankAccountDetails ? `Bank Account: ${contact.bankAccountDetails}` : null,
            contact.taxNumber ? `Tax Number: ${contact.taxNumber}` : null,
            contact.accountsReceivableTaxType ? `AR Tax Type: ${contact.accountsReceivableTaxType}` : null,
            contact.accountsPayableTaxType ? `AP Tax Type: ${contact.accountsPayableTaxType}` : null,
            ...
            ...contact.addresses?.map(addr => 
              `  ${addr.addressType}: ${[
                ...
                addr.postalCode,
                addr.country
              ].filter(Boolean).join(', ')}${addr.attentionTo ? ` (Attn: ${addr.attentionTo})` : ''}`
            ) || [],
            ...
            contact.contactGroups?.length ? `Groups: ${contact.contactGroups.map(g => g.name).join(', ')}` : null,
...
server.tool(
  ...
  async (_args: {}, _extra: { signal: AbortSignal }) => {
      ...
      content: [
        ...
        ...result.invoices?.map(invoice => ({
          ...
          text: [
            ...
            invoice.contact ? `Contact: ${invoice.contact.name} (${invoice.contact.contactId})` : null,
    ...
    Line Amount: ${item.lineAmount || 0}${item.taxType ? `
    Tax Type: ${item.taxType}` : ''}${item.taxAmount ? `
    Tax Amount: ${item.taxAmount}` : ''}${item.accountCode ? `
    Account Code: ${item.accountCode}` : ''}${item.discountRate ? `
    Discount Rate: ${item.discountRate}` : ''}${item.tracking?.length ? `
    Tracking: ${item.tracking.map(t => `${t.name}: ${t.option}`).join(', ')}` : ''}`
            ...
            invoice.totalDiscount ? `Total Discount: ${invoice.totalDiscount}` : null,
    ...
    Amount: ${payment.amount || 0}${payment.reference ? `
    Reference: ${payment.reference}` : ''}${payment.hasAccount ? `
    Has Account: Yes` : ''}${payment.hasValidationErrors ? `
    Has Validation Errors: Yes` : ''}`
            ) || [],
            invoice.prepayments?.length ? 'Prepayments:' : null,
            ...invoice.prepayments?.map(prepayment =>
              `  - ID: ${prepayment.prepaymentId}
    Date: ${prepayment.date || 'Unknown'}${prepayment.reference ? `
    Reference: ${prepayment.reference}` : ''}`
            ) || [],
            invoice.overpayments?.length ? 'Overpayments:' : null,
            ...invoice.overpayments?.map(overpayment =>
    ...
    Number: ${note.creditNoteNumber || 'Unknown'}${note.reference ? `
...
server.tool(
  ...
  async ({ name, email, phone }: { name: string; email?: string; phone?: string }, _extra: { signal: AbortSignal }) => {
    try {
...
server.tool(
  ...
  async ({ contactId, description, quantity, unitAmount, accountCode, taxType, reference }: { 
    contactId: string;
    description: string;
    quantity: number;
    unitAmount: number;
    ...
    const result = await createXeroInvoice(contactId, description, quantity, unitAmount, accountCode, taxType, reference);
    if (!result.success) {
      return {
        content: [
          {
